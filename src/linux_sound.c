...
void audio_callback(void *userdata, u8 *stream, int len) {
    ...
    ASSERT(sound->bytes_per_sample == 2 * sizeof(s16));

    u32 samples_to_read = MIN(samples_requested, sound->samples_per_second);
    s64 region1_samples_available = sound->samples_per_second - sound->running_sample_index_for_reading;
    ...
    s16* dest_sample = (s16*)stream;
    s16* source_sample = (s16*)(sound->secondary_buffer + sound->running_sample_index_for_reading * sound->bytes_per_sample);
    ...
    if (wraps_around) {
        ...
        source_sample = (s16*)sound->secondary_buffer;
...
static void linux_fill_sound_buffer(linux_sound_output_t* sound, game_sound_buffer_t* source_buffer) {
    ...
    s64 region1_samples_available = sound->samples_per_second - sound->running_sample_index_for_writing;
    ...
    s16* dest_sample = (s16*) (sound->secondary_buffer + sound->running_sample_index_for_writing * sound->bytes_per_sample);
    ...
    if (wraps_around) {
        ...
        dest_sample = (s16*)sound->secondary_buffer;
...
static void linux_produce_sound_for_frame(app_state_t* app_state, sdl_sound_output_t* sound, game_sound_buffer_t* game_sound_buffer, s64 flip_clock) {
